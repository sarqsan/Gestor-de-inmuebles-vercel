...
| HEAD canónico | `2332bb1` — `docs(gap-r1): pliego de traspaso GAP-R1 a Arena B (MAPA §12.6, anexo CONTRATO)` (solo documentación; último commit con cambios en el árbol de código/tests: `c39f183`, ORDEN 16) |
| Último commit canónico | `2332bb1` (docs) ← `c39f183` (`test(gap-r4)`, GAP-R4 CERRADO) ← `ce67c09`/`3f1509c` (ORDEN 15) ← … ← `d606ff8` (integración selectiva BLOQUE D) |
...
| Tests | **835/835** (35 ficheros vitest, regresión global tras ORDEN 16: 638 + 197 de GAP-R4) + **92/92** (`npm run test:bloque-b`) + **82 PASS** (`npm run test:bloque-c`) + **64/64** (`npm run test:bloque-e`) — re-verificados 2026-09-21 |
