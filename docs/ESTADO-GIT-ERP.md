...
| HEAD canónico | `c90777f` — `fix(gap3,gap6): filtrar gastos por fecha efectiva (fechaDevengo/fechaPago), no por alias g.fecha (correctiva D2)` |
| Último commit canónico | `c90777f` (O17 D2) ← `e3e563a`/`2332bb1` (docs) ← `c39f183` (`test(gap-r4)`, GAP-R4 CERRADO) ← `ce67c09`/`3f1509c` (ORDEN 15) ← … ← `d606ff8` (integración selectiva BLOQUE D) |
