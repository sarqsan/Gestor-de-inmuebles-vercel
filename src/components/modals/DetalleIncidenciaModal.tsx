...
import { scopeFromUsuario, subscribeTrabajosProfesionales, subscribePresupuestosProfesionales } from '../../lib/firebase';
...
export const DetalleIncidenciaModal: React.FC<DetalleIncidenciaModalProps> = ({
  ...
  useEffect(() => {
    ...
    // Consulta acotada por rol (propietario/profesional): nunca la colección
    // completa de órdenes de trabajo de otros propietarios.
  ...
  const handleEjecutarAnalisisIA = async () => {
    ...
    try {
      const resp = await fetch('/api/analizar-incidencia-ia', {
        ...
        // Se envían SÓLO los campos necesarios para el diagnóstico: la API
        // valida y sanea lo que recibe y no accede a Firebase. No se envían
        // datos personales del inquilino, notas internas, histórico ni el
        // número de póliza.
        body: JSON.stringify({
          incidencia: {
          ...
          polizas: polizas
            .filter((p) => !incidencia.inmuebleId || !p.inmuebleId || p.inmuebleId === incidencia.inmuebleId)
            .slice(0, 10)
            .map((p) => ({
