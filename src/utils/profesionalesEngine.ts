...
/**
 * Totales económicos de un presupuesto a partir de sus partidas.
 * El IVA es orientativo y se aplica sobre la base imponible.
 */
export function calcularTotalesPresupuesto(
  partidas: PartidaPresupuesto[],
  porcentajeIva: number = 21
): { importeBase: number; iva: number; importeTotal: number } {
  const base = (partidas || []).reduce((acc, p) => {
    const cantidad = Number(p?.cantidad) || 0;
    const precio = Number(p?.precioUnitario) || 0;
    const importe = p?.importe !== undefined ? Number(p.importe) || 0 : cantidad * precio;
    return acc + importe;
  }, 0);
  const importeBase = Number(base.toFixed(2));
  const iva = Number(((importeBase * (Number(porcentajeIva) || 0)) / 100).toFixed(2));
  return { importeBase, iva, importeTotal: Number((importeBase + iva).toFixed(2)) };
}
...
export function calcularMetricasGeneralesTrabajos(
  ...
  const mediaPuntuacionGlobal =
    valoraciones.length > 0
      ? Number(
          (
            valoraciones.reduce((acc, v) => acc + (Number(v.puntuacion) || 0), 0) /
            valoraciones.length
          ).toFixed(1)
        )
      : 0;
  ...
  return {
    ...
    // Alias explícitos que consume la UI de Profesionales & Obras
