...
describe('FASE 1.4 · D. `syncAuthIndex`: el cliente escribe el espejo que leen las reglas', () => {
  ...
  it('D.6 el espejo es el único canal de identidad: no hay un segundo sistema en el cliente', () => {
    ...
    // legítimo de la ruta, no del espejo: se admite ÚNICAMENTE si (a) cita la colección
    // padre solo como segmento de esa subcolección y (b) no realiza ninguna E/S sobre el
    // documento `usuarios_auth/{uid}` en sí (ni `doc(db, 'usuarios_auth', uid)` de dos
    // segmentos, ni `setDoc/updateDoc/deleteDoc` sobre él).
    const esConsumidorLegitimoDeSubcoleccion = (f: string): boolean => {
      const src = readFileSync(f, 'utf8');
      // barrel de §6 F3: solo re-exporta las constantes de `progreso.ts` (sin E/S ni literal)
      if (/experiencia[\\/]index\.ts$/.test(f) && !/['"`]usuarios_auth['"`]/.test(src) && !/\bdoc\(|\bcollection\(/.test(src)) return true;
      if (!/progreso_tutoriales|SUBCOLECCION_PROGRESO_TUTORIALES/.test(src)) return false;
      // acceso directo al documento del espejo: `doc(db, 'usuarios_auth', <uid>)` sin más segmentos
      if (/doc\(\s*db\s*,\s*['"`]usuarios_auth['"`]\s*,\s*[^,()]+\)/.test(src)) return false;
    ...
    expect(subcoleccion).toEqual(['index.ts', 'progreso.ts', 'progresoTutorialesFirestore.ts']);
    // authService.ts sigue escribiendo el espejo en su ruta de dos segmentos (no en la subcolección)
    const auth = readFileSync(path.resolve(dir, 'lib/authService.ts'), 'utf8');
    expect(auth).toContain("doc(db, 'usuarios_auth', fb.uid)");
  });
});
